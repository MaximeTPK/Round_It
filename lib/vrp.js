...
// ─── Service time calculé par order ───────────────────────────────
//
// Base : 15 min minimum (picking et delivery)
// Par order (volume individuel) :
//   0 - 0.1 m³  → +3 min
//   0.1 - 0.2 m³ → +7 min
//   0.2 - 1 m³   → +10 min
//   ≥ 1 m³       → +15 min
//   ≥ 1.5 m³     → flag "2 drivers"
//
...
export function computeServiceTime(stop) {
  ...
  // Base 15 min pour le premier order (déjà inclus dans BASE)
  // Chaque order supplémentaire ajoute du temps selon son volume
...
export function checkNeedsTwoDrivers(stop) {
  const volumes = stop.orderVolumes || []
  return volumes.some(v => v >= 1.5)
}

export function getTwoDriversOrders(stop) {
  const volumes = stop.orderVolumes || []
  const orders = stop.orders || []
  const result = []
  for (let i = 0; i < volumes.length; i++) {
    if (volumes[i] >= 1.5) {
      result.push({ orderId: orders[i] || '?', volume: volumes[i] })
    }
  }
  return result
}
...
function isFeasible(depot, route, startMin, endMin, truckCapacity) {
  // Contrainte horaire + fenêtres
  let time = startMin
  let prev = depot
  for (const stop of route) {
    time += travelTime(prev, stop)
    // Attente si arrivée avant la fenêtre
    if (stop.timeFrom != null && time < stop.timeFrom) {
      time = stop.timeFrom
    }
    // Vérifier fenêtre stricte
...
function computeRouteStats(depot, route, startMin) {
  ...
  for (const stop of route) {
    ...
    // Vérifier si hors fenêtre (pour affichage)
    ...
    detailed.push({
      ...
      lateBy,
      needsTwoDrivers: checkNeedsTwoDrivers(stop),
    })
...
export function solveMultiDayVRP(
  ...
  for (let d = 0; d < numDays && remaining.length > 0; d++) {
    ...
    // Phase 5 : Réordonner par priorité — high en premier, puis medium, puis low
    // Au sein de chaque groupe, on garde l'ordre optimisé par distance
    for (let t = 0; t < numTrucks; t++) {
      if (truckRoutes[t].length < 2) continue
      const highStops = truckRoutes[t].filter(s => (s.priority || 'medium') === 'high')
      const medStops = truckRoutes[t].filter(s => (s.priority || 'medium') === 'medium')
      const lowStops = truckRoutes[t].filter(s => (s.priority || 'medium') === 'low')

      // Optimiser chaque groupe individuellement avec 2-opt
      const orderedHigh = highStops.length >= 3 ? twoOpt(depot, highStops) : highStops
      const lastHigh = orderedHigh.length > 0 ? orderedHigh[orderedHigh.length - 1] : depot
      const orderedMed = medStops.length >= 3 ? twoOpt(lastHigh, medStops) : medStops
      const lastMed = orderedMed.length > 0 ? orderedMed[orderedMed.length - 1] : lastHigh
      const orderedLow = lowStops.length >= 3 ? twoOpt(lastMed, lowStops) : lowStops

      const reordered = [...orderedHigh, ...orderedMed, ...orderedLow]

      // Vérifier faisabilité — si pas faisable, garder l'ordre d'avant
      if (isFeasible(depot, reordered, startMin, endMin, truckCapacity)) {
        truckRoutes[t] = reordered
      }
    }

    // Phase 6 : Résultats du jour
